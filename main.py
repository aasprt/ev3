...
class Robot:
    def __init__(self, kp = 1.9, ki = 0.007, kd = 10):
    ...
    def follow_line(self):
        ...
        if (self.color_sensor_left.reflection() >= 70 and self.color_sensor_right.reflection() >= 70) or (self.color_sensor_left.reflection() <= 15 and self.color_sensor_right.reflection() <= 15):
    ...
    def do_intersection(self, direction):

        self.motor_left.dc(20)
...
while not inside_room:
    #if UltrasonicSensor(Port.S3).distance() <= 100:
        #Motor(Port.A).hold()
        #Motor(Port.D).hold()
        #ev3.speaker.beep()
